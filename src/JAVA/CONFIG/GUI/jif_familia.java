...
import JAVA.CONFIG.LOGICA.evt_grid_familia;
...
public class jif_familia extends javax.swing.JInternalFrame {

    pnl_datos_familia lo_pnl_datos_familia = new pnl_datos_familia();
    evt_datos_familia lo_evt_datos_familia = new evt_datos_familia();
    pnl_grid_familia lo_pnl_grid_familia = new pnl_grid_familia();
    evt_grid_familia lo_evt_grid_familia = new evt_grid_familia();
    ...
    String ls_codigo;
    String ls_modulo = "CONFIG", ls_capa = "GUI", ls_clase = "jif_familia";
    ...
    private void formulario() {
        lo_pnl_datos_familia.setBounds(0, 0, 450, 350);
        lo_pnl_grid_familia.setBounds(0, 7, 500, 350);
        lo_pnl_opciones_1.setBounds(0, 0, 350, 100);
        lo_pnl_opciones_2.setBounds(0, 0, 450, 100);

        PNL_datos.add(lo_pnl_datos_familia);
        PNL_visor.add(lo_pnl_grid_familia);
        PNL_opcion_1.add(lo_pnl_opciones_1);
        PNL_opcion_2.add(lo_pnl_opciones_2);

        modelo = (DefaultTableModel) lo_pnl_grid_familia.TBL_marca.getModel();

        lo_evt_grid_familia.datos_tabla(modelo);
        ...
        lo_evt_grid_familia.evento_press(lo_pnl_grid_familia, KeyEvnt);
    ...
    private void evt_eliminar() {
        try {
            if (go_fnc_mensaje.get_respuesta(0, "¿DESEA ELIMINAR FAMILIA " + lo_pnl_grid_familia.TBL_marca.getValueAt(lo_pnl_grid_familia.TBL_marca.getSelectedRow(), 1) + "?") == 0) {
                ls_codigo = lo_pnl_grid_familia.TBL_marca.getValueAt(lo_pnl_grid_familia.TBL_marca.getSelectedRow(), 0).toString();
    ...
    private void evt_editar() {
        try {
            ...
            gs_parametros[1] = lo_pnl_grid_familia.TBL_marca.getValueAt(lo_pnl_grid_familia.TBL_marca.getSelectedRow(), 1).toString();
            
            go_dlg_editar_familia = new dlg_editar_familia(null, true);
            go_dlg_editar_familia.setVisible(true);
            lo_evt_grid_familia.datos_tabla(modelo);
        } catch (Exception e) {
    ...
    private void evt_exportar() {
        go_evt_exportar.exportar(lo_pnl_grid_familia.TBL_marca);
    }
    ...
    KeyListener KeyEvnt = new KeyListener() {
        ...
        public void keyPressed(KeyEvent ke) {
            ...
            if (ke.getKeyCode() == KeyEvent.VK_ENTER) {
                ...
                if (ke.getSource() == lo_pnl_grid_familia.TXT_dato) {
                    if (lo_pnl_grid_familia.TBL_marca.getRowCount() != 0) {
                        lo_pnl_grid_familia.TBL_marca.requestFocus();
                        lo_pnl_grid_familia.TBL_marca.changeSelection(0, 0, false, false);
                    } else {
                        lo_pnl_grid_familia.TXT_dato.requestFocus();
                    }
                }
            }
        }
        ...
        public void keyReleased(KeyEvent ke) {
            if (ke.getSource() == lo_pnl_grid_familia.TXT_dato) {
                go_fnc_filtrar_tablas.filtro(modelo, lo_pnl_grid_familia.TBL_marca, lo_pnl_grid_familia.TXT_dato.getText(), 1);
            }
        }
    };
