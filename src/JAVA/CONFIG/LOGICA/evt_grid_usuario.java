...
import JAVA.CONFIG.GUI.pnl_grid_usuario;
import java.awt.event.KeyListener;
import java.sql.ResultSet;
import javax.swing.table.DefaultTableModel;

public class evt_grid_usuario {
    ...
    public void datos_tabla(DefaultTableModel modelo) {
        ...
        int a = 0;
        try {
            lq_rs = go_dao_usuario.SLT_datos_usuario(1, 0);
            if (lq_rs != null) {
                do {
                    modelo.addRow(new Object[]{""});
                    modelo.setValueAt(lq_rs.getString(1), a, 0);
                    modelo.setValueAt(lq_rs.getString(3), a, 1);
                    modelo.setValueAt(go_fnc_opera_campos.desencriptar(lq_rs.getString(4)), a, 2);
    ...
    public KeyListener evento_press(pnl_grid_usuario OBJ_pnl, KeyListener KeyEvnt) {
        OBJ_pnl.TXT_dato.addKeyListener(KeyEvnt);     
        OBJ_pnl.TBL_usuario.addKeyListener(KeyEvnt);
        return KeyEvnt;
    }
}
